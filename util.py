...
def print_report(image: NDArray, visited: set, times, max_err_threshold: float):
    print(f"Progress: {len(visited)}/{image.shape[0] * image.shape[1]} : {((len(visited)/(image.shape[0] * image.shape[1])) * 100):.2f}%")
...
"""
def create_new_image(image, scale, seed_size=10):
    # SCALE factor
